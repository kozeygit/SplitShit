...
export const billPayers = sqliteTable(
    "bill_payers",
    {
        id: integer("id").primaryKey({ autoIncrement: true }),
        billId: integer("bill_id").references(() => bills.id, {
        ...
        addedWithGroup: integer("added_with_group", { mode: "boolean" })
            .notNull()
            .default(false),
    },
    (table) => [unique("bill_payer_unique").on(table.billId, table.payerId)],
);
...
export const groupPayers = sqliteTable(
    "group_payers",
    {
        id: integer("id").primaryKey({ autoIncrement: true }),
        groupId: integer("group_id").references(() => groups.id, {
    ...
    (table) => [unique("group_payer_unique").on(table.groupId, table.payerId)],
);
