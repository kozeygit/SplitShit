...
export const useBillStore = create<BillState>((set, get) => ({
  ...
  setOriginalBill: (bill) =>
    set({ originalBill: bill ? cloneDeep(bill) : undefined }), // Clone or undefined
  setEditedBill: (bill) =>
    set({ editedBill: bill ? cloneDeep(bill) : undefined }), // Clone or undefined
  resetEditedBill: () =>
    set((state) => ({
      editedBill: state.originalBill
        ? cloneDeep(state.originalBill)
        : undefined, // Clone or undefined
    })),
}));
