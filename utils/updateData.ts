...
export const setBillComplete = async (
    ...
    try {
        const insertedBill: { id: number; complete: number }[] = await db
            ...
            .returning({
                id: schema.bills.id,
                complete: schema.bills.complete,
            });
...
export const updateBill = async (bill: Bill): Promise<number> => {
    try {
        ...
        await updateBillItems(bill);
        await updateBillPayers(bill);
        await updateItemAssignments(bill);
        await updateBillImagePath(bill.id, bill.imagePath);
...
const updateItemAssignments = async (bill: Bill): Promise<void> => {
    ...
    for (const item of bill.items) {
        ...
        for (const oldAssignment of oldAssignments) {
            ...
            for (const bp of billPayers) {
                if (bp.id == oldAssignment.billPayerId) {
                    found = true;
                    if (
                        item.assignedTo.find(
                            (obj) => obj.payerId == bp.payerId,
                        ) === undefined
                    ) {
                        found = false;
                    }
        ...
        for (const bp of billPayers) {
            ...
            const assignedTo = item.assignedTo.find(
                (obj) => obj.payerId == bp.payerId,
            );
...
export const updateGroupPayers = async (group: Group): Promise<void> => {
    ...
    for (const payer of group.payers) {
        const isOldPayer = oldPayers.some(
            (oldPayer) => oldPayer.id == payer.id,
        );
        if (isOldPayer) {
            continue;
        }
...
export const updateBillImagePath = async (
    billId: number,
    imagePath?: string,
): Promise<void> => {
    await db
